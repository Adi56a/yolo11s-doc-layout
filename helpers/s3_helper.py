...
class S3StorageHelper:
    ...
    def __init__(
        ...
        if self.use_local_storage:
            ...
            print(
                f"[STORAGE] Running in local mock mode. Target dir: {self.local_storage_dir.resolve()}"
            )
        else:
            if boto3 is None:
                raise ImportError(
            ...
            self.bucket_name = bucket_name or os.getenv(
                "S3_BUCKET_NAME", "edtech-b2b-media"
            )
            ...
            print(
                f"[STORAGE] Connected to AWS S3. Bucket: {self.bucket_name}, Region: {self.region}"
            )
    ...
    def upload_file(self, local_file_path: str | Path, target_key: str) -> str:
        ...
        if self.use_local_storage:
            ...
            # Return absolute local file URL
            ...
            try:
                ...
                raise RuntimeError(
                    "AWS Credentials not found or invalid. Please check your .env configuration."
                )
            except Exception as e:
                raise RuntimeError(
                    f"Failed S3 upload for key '{target_key}': {e}"
                )
