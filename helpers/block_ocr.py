...
class BlockOCR:
    ...
    def __init__(self, engine_name: str | None = None) -> None:
        ...
        if self.engine_name == "easyocr":
            try:
                ...
                print(
                    "[OCR] [WARNING] easyocr package not found. Falling back to Mock OCR."
                )
                self.engine_name = "mock"
            except Exception as e:
                print(
                    f"[OCR] [WARNING] EasyOCR failed to load: {e}. Falling back to Mock OCR."
                )
                self.engine_name = "mock"
            ...
            try:
                ...
                print(
                    "[OCR] [WARNING] pytesseract package not found. Falling back to Mock OCR."
                )
                self.engine_name = "mock"
            except Exception as e:
                print(
        ...
        if self.engine_name == "mock":
            print(
                "[OCR] Running in Mock OCR mode (no external OCR library required)."
            )
    ...
    def extract_text(
        ...
        is_question_anchor and block_idx are used to generate realistic mock texts
        when in mock mode.
        """
        ...
        if self.engine_name == "easyocr" and self.reader is not None:
            try:
                ...
                print(
                    f"[OCR] EasyOCR run failed: {e}. Falling back to mock text extraction."
                )
            ...
            try:
                ...
                print(
                    f"[OCR] PyTesseract run failed: {e}. Falling back to mock text extraction."
                )
        ...
        if is_question_anchor:
            # Generate a realistic question label e.g., "Question 1"
